...
1. **LangGraph agent with LLM-driven tool use.** The model decides when to call tools — no hardcoded sequences. Tools:
   ...
   - `web_search` — the required external tool (**Tavily**), for general cooking questions and gaps the local dataset can't fill (substitutions, techniques).
...
3. **Lightweight memory across sessions** for *food preferences*: cuisines they like, dislikes, and **food avoidances** — including stated dietary preferences ("I'm vegetarian") and allergen avoidances ("no shellfish"). Keyed by a user id. Medical conditions are never stored. This is Marcus's "feels like a relationship" bar, scoped to what Legal allows (see Contradiction 4).
...
4. **Memory vs data retention — Marcus vs Diane.** We draw the line at *food preference vs. medical fact*. We **persist food avoidances** — cuisine likes/dislikes, "vegetarian," and allergen avoidances like "no shellfish" — because Marcus's flagship example ("don't suggest shrimp if I said shellfish last week") is the whole point of memory, and a user-stated food avoidance is a preference, not a diagnosis. We **never persist medical conditions** (diabetes, pregnancy, medications) and the assistant never adapts food to them or makes nutritional-appropriateness claims — it acknowledges generically and points to a professional. This gives Marcus the continuity he cares about while honoring Diane's "no medical data, no medical advice." The memory extractor enforces this split in code, and a delete endpoint (`DELETE /api/memory/{user}`) provides the right-to-delete story.
